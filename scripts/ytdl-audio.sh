#!/bin/bash
# ytdl-audio.sh — download a YouTube video's audio track using yt-dlp.
#
# I/O contract:
...
usage() {
    cat <<'EOF'
Usage: ytdl-audio.sh URL [--path PATH] [--filename NAME]

Download a YouTube video's audio track using yt-dlp.

Always produces one audio file. No re-encoding, no post-processing —
whatever YouTube serves as the best audio-only stream is what gets
written.
...
---------------
  ...
  Command substitution is therefore always safe:
      f=$(ytdl-audio.sh URL) && echo "Got: $f"

Examples
--------
  ytdl-audio.sh https://www.youtube.com/watch?v=dQw4w9WgXcQ
  ytdl-audio.sh https://youtu.be/dQw4w9WgXcQ -p Podcasts
  DEBUG=true ytdl-audio.sh https://youtu.be/dQw4w9WgXcQ
EOF
}
...
debug "→ Downloading audio to $DEST_PATH"
