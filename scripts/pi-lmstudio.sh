...
# Launch pi against a local LM Studio model
pix() {
    ...
    # Inject selected model into ~/.pi/agent/models.json under the "lmstudio" provider.
    # pi uses this file (not ANTHROPIC_BASE_URL) to resolve custom endpoints.
    # We preserve any existing config and restore it on exit.
    local models_json_backup=""
    local models_json_existed=false
    if [[ -f "$PI_MODELS_JSON" ]]; then
        models_json_existed=true
        models_json_backup=$(cat "$PI_MODELS_JSON")
    fi

    _pix_restore_models_json() {
        if [[ "$models_json_existed" == "true" ]]; then
            echo "$models_json_backup" > "$PI_MODELS_JSON"
        else
            rm -f "$PI_MODELS_JSON"
        fi
    }
    trap '_pix_restore_models_json' EXIT INT TERM

    mkdir -p "$(dirname "$PI_MODELS_JSON")"
    local base_config="{}"
    [[ -n "$models_json_backup" ]] && base_config="$models_json_backup"

    # Merge/upsert the lmstudio provider with the selected model
    local updated_config
    updated_config=$(echo "$base_config" | jq \
        --arg id "$selected" \
        --argjson ctx "$selected_ctx" \
        '
        .providers.lmstudio = {
            ...
            "models": [{ "id": $id, "contextWindow": $ctx }]
        }
        ')
    echo "$updated_config" > "$PI_MODELS_JSON"

    echo
    pi --model "lmstudio/$selected"

    # Restore models.json and clean up trap
    trap - EXIT INT TERM
    _pix_restore_models_json
    unset -f _pix_restore_models_json
